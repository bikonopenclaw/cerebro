...
class ArxClient:
    ...
    def login(self):
        ...
        data = self.call('Login', {'partner': partner, 'username': username, 'password': password}, visa=False)
        try:
            self.partner_id = int(data['result']['result']['PartnerId'])
    ...
    def get_partner_info(self):
        if self.partner_id is None:
            raise RuntimeError('Login ARX/Cove necessario antes da consulta')
        query = {
        ...
        return self.call('EnumerateAccountStatistics', {'query': query})
