...
import argparse,datetime,fcntl,hashlib,json,os,re,shutil,sys,tempfile
...
class Runtime:
    def __init__(self,config):
        ...
        self.model_dir=Path(config['model_dir']).resolve();self.config=config;self._model=None
    ...
    def model(self):
        if self._model is None:
            self._model=TextEmbedding(MODEL,specific_model_path=str(self.model_dir),cache_dir=str(self.data/'model-cache'),
...
def main():
    ...
    p.add_argument('--refresh-only',action='store_true');p.add_argument('--scope',choices=['knowledge','all'],default='knowledge')
    ...
    try:
        runtime=Runtime(json.loads(a.config.read_text()))
        if a.refresh_only:
