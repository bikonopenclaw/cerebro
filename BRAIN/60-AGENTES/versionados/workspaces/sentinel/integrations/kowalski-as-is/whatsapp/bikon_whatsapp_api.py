...
SENSITIVE_KEYS = {
    'token', 'accessToken', 'access_token', 'jwt_token', 'metaAccessToken',
    'authorization', 'Authorization', 'secret', 'password', 'senha'
}
...
def sanitize(obj):
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if k in SENSITIVE_KEYS or 'token' in k.lower() or 'secret' in k.lower() or 'senha' in k.lower() or 'password' in k.lower():
                clean[k] = mask_value(v)
            else:
                clean[k] = sanitize(v)
        return clean
...
def print_result(res):
    print(json.dumps(sanitize(res), ensure_ascii=False, indent=2))
    if not res.get('ok'):
        sys.exit(1)
