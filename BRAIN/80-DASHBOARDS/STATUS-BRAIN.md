...
- OpenClaw - Provimento 213 registrado como projeto governado por checkpoints: EP-01 documental `READY`, EP-02 Inventory Freeze corrigido em commit `61b1dc8...`, runtime CPython `3.14.6` formalizado, baseline técnico do primeiro ciclo homologado em `e7be4903...`, publicação canônica por bundle/manifest homologada no Drive, plano formal de teste de restauração proposto, aquisição de metadata autoritativa, reach read-only congelado, discovery de provider bloqueada por falta de evidência AWS/Azure/GCP, adaptive interview/dashboard validado e multi-Serventia HTML/PT-BR/PDF preparado. Target, preflight, restore, deploy, execução em serventia real, entrevista externa, envio de PDF e recorrência continuam bloqueados sem Approval explícito.
- OpenClaw DRE v1: repositório canônico em `/opt/openclaw/src/openclaw`, target freeze DRE preservado, commit local `3b4bff00...` com `75/75` testes pre-install e rollback `PASS`; capability não instalada porque o launcher final não era relocatable.
