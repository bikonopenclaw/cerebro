...
No fechamento histórico ICD multi-CNS, a primeira revisão de 13 testes não bastou: export e evidência ainda usavam seleção global, e leitura podia reconstruir estado. A correção passou a servir bytes por CNS e impedir rebuild no startup/GET. Após promoção autorizada, o serviço user em 9213 ficou ativo e a checagem externa registrou seis rotas 200, traversal 404, idempotência e zero mutação; PDFs retornavam 403 deliberadamente nessa etapa read-only. Esse 403 não deve ser confundido com regressão depois que uma etapa posterior autorizou PDF 200. Consulta ao system manager não provava serviço user parado. A cauda iniciou compatibilidade de prefixos /prov213, sem resultado final nela. Fonte: unidades 9542.
...
No gate ICD visual/analytics de 08/08, 200 testes passaram, mas o primeiro harness independente falhou ao tratar HTML bytes como str; o seguinte presumiu records/cns e campo cns obrigatório em detail_rows. A inspeção mostrou catálogo entries/CNS_inventory e validou isolamento pelo record/rotas corretos; o falso diagnóstico de cross-CNS foi retirado. Corrigir o checker contra o schema efetivo e provar os limites sem enfraquecer o contrato da aplicação. O staging terminou PASS com GET_MUTATION_COUNT=0; a nova checagem externa iniciada depois da promoção ainda não tinha resultado nesta fonte, por isso não herda esse PASS. Fonte: unidades 9564.

A engenharia CPIW multi-CNS posterior removeu o hard-bind produtivo a 023689 e passou 218 testes, regressão e simulações parciais sem mutação real. O tar final aceito foi f2475647a5654a9045da77f04e915787cd29ab5abd9c8347314e4cddcdad4320, substituindo o pacote inicial. O pacote produtivo seguinte declarou commits parciais para 023218 e 023879; a revisão mostrava 023218 COMMITTED com 28/47 respostas, 19 pendentes, retomada em Q-CONTROL-P-03 e replay PASS_NO_OP. Ainda faltava concluir a conferência de 023879 e emitir aceite independente, portanto PASS do executor não era esse aceite. Percentuais 59,57% e 61,70% descreviam entrevista parcial, não conformidade integral; respostas históricas não deviam ser reperguntadas. Ressalva do hash arqueológico corrigido precisa permanecer na linhagem, sem manual apply ou novo commit por inferência. Fonte: unidades 8798.

Após a homologação técnica EP-02, o kernel validava envelopes e identidades, mas não originava eventos de domínio nem implementava ciclo de negócio. A preparação seguinte exigia perfil de controle congelado, evidência, critério de avaliação, ação corretiva e Approval próprios. Na busca documental, Registro_Teste_Restauracao_Backup_4Oficio_Vitoria_v1.0.md era modelo em branco, com status a preencher: sua existência não provava teste realizado. Os outcomes, periodicidades e requisitos do contrato candidato ainda estavam sob validação de suporte normativo nessa cauda; não promovê-los a regra aprovada por mera transcrição. O Business Completion posterior é etapa distinta que supera a ausência inicial de contrato. Fonte: unidades 8867.

Na engenharia histórica CPIW multi-CNS, remover hard-bind produtivo a 023689 não significava apagar toda ocorrência desse CNS: referências restantes em fixture congelada de regressão, marcador shadow e evidência histórica eram legítimas. Conferir o caminho ativo e a igualdade CNS entre preview, autoridade, preflight, apply, rollback e validação, com staging por CNS. O pacote inicial tinha template de token com placeholder; o tar final repacotado sem duplicatas recebeu hash f2475647a5654a9045da77f04e915787cd29ab5abd9c8347314e4cddcdad4320 e precisava desse vínculo exato para ação produtiva futura. PASS de 218 testes e simulações parciais não era commit; a revisão de aceitação produtiva iniciou separadamente e ainda não concluiu nessa cauda. Fonte: unidades 8795.

Na recuperação de 21/08, testes 32/32 e 253/253, HTML atualizado e canário HTTP 200 não evitaram a rejeição real do Owner. A rodada B20260821T103100Z continuou sem carregar os dois intents. A restauração posterior usou Brand Composition como autoridade visual, reconciliação do processo órfão com serviço user em 9213, build B20260821T112000Z, 36/36 e 257/257 testes e seis serventias autorizadas. Traces de navegador e loopback CONTENT_RENDERED ainda não eram aceite iPhone. O checkpoint aguardava taps reais; o fechamento posterior de 26/27 de agosto já documentado supera essa pendência. Consulta ao system manager ou listener manual, isoladamente, não prova ausência ou persistência do serviço user. start_param seleciona a visão; autorização depende de initData validado no servidor, membership e entitlement. Fonte: unidades 32839, 32854.
