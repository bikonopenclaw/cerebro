...
NUMERO_PARCELAMENTO="${NUMERO_PARCELAMENTO:-2}"
SERVICO="OBTERPARC164"
SISTEMA="PARCSN"
VERSAO="${VERSAO:-1.0}"
...
PAYLOAD="$(CNPJ="$CNPJ" NUMERO_PARCELAMENTO="$NUMERO_PARCELAMENTO" SISTEMA="$SISTEMA" SERVICO="$SERVICO" VERSAO="$VERSAO" python3 - <<'PY'
import json, os
cnpj=os.environ['CNPJ']
try:
    numero=int(os.environ['NUMERO_PARCELAMENTO'])
except ValueError:
    raise SystemExit('NUMERO_PARCELAMENTO precisa ser número inteiro')
pedido={
  'idSistema': os.environ['SISTEMA'],
  'idServico': os.environ['SERVICO'],
  'versaoSistema': os.environ['VERSAO'],
  'dados': json.dumps({'numeroParcelamento': numero}, ensure_ascii=False)
}
body={
...
echo "Parcelamento: ${NUMERO_PARCELAMENTO}"
echo "Dados internos: {\"numeroParcelamento\":${NUMERO_PARCELAMENTO}}"
